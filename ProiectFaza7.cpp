...
class LivadaEco : public Livada
{
private:
	char* ecologieFolosita;
	float calitateSol;

public:
	LivadaEco() : Livada(12, 2999)
	{
		this->ecologieFolosita = new char[strlen("Compostare si reciclare") + 1];
		strcpy(this->ecologieFolosita, "Compostare si reciclare");
		this->calitateSol = 8.8;
	}

	LivadaEco(const char* ecologieFolosita, float calitateSol, float suprafata, int nrPomi, float* productiePomi) : Livada(13, suprafata, nrPomi, productiePomi)
	{
		this->ecologieFolosita = new char[strlen(ecologieFolosita) + 1];
		strcpy(this->ecologieFolosita, ecologieFolosita);
		this->calitateSol = calitateSol;
	}

	LivadaEco(const LivadaEco& le) : Livada(le)
	{
		this->ecologieFolosita = new char[strlen(le.ecologieFolosita) + 1];
		strcpy(this->ecologieFolosita, le.ecologieFolosita);
		this->calitateSol = le.calitateSol;
	}

	~LivadaEco() {
		if (this->ecologieFolosita != nullptr)
		{
			delete[]this->ecologieFolosita;
		}
	}

	LivadaEco& operator= (const LivadaEco& le)
	{
		if (this != &le)
		{
			Livada::operator=(le);
	...
	friend ostream& operator<<(ostream& out, const LivadaEco& le)
	{
		out << Livada(le) << endl;
		out << le.ecologieFolosita << endl;
		out << le.calitateSol << endl;
		return out;
	}

	char* getEcologieFolosita() const
	{
		return ecologieFolosita;
	}

	float getCalitateSol() const
	{
		return calitateSol;
	}

	void setCalitateSol(float calitateSol)
	{
		if (this->calitateSol != 0)
		{
			this->calitateSol=calitateSol;
		}
	}
...
class PomicultorAvansat : public Pomicultor
{
private:
	int aniStudiu;
	char* tehnologieInvatata;

public:

	PomicultorAvansat(): Pomicultor()
	{
		this->aniStudiu = 4;
		this->tehnologieInvatata = new char[strlen("Agro-Ecologie") + 1];
		strcpy(this->tehnologieInvatata, "Agro-Ecologie");
	}

	PomicultorAvansat(int aniStudiu, const char* tehnologieInvatata, const char* denumirePomicultor, int varsta, bool areUtilaje) : Pomicultor(25, denumirePomicultor, varsta, areUtilaje)
	{
		this->aniStudiu = aniStudiu;
		this->tehnologieInvatata = new char[strlen(tehnologieInvatata) + 1];
		strcpy(this->tehnologieInvatata, tehnologieInvatata);
	}

	PomicultorAvansat(const PomicultorAvansat& pa): Pomicultor(pa)
	{
		this->aniStudiu = pa.aniStudiu;
		this->tehnologieInvatata = new char[strlen(pa.tehnologieInvatata) + 1];
		strcpy(this->tehnologieInvatata, pa.tehnologieInvatata);
	}

	~PomicultorAvansat()
	{
		if (this->tehnologieInvatata != nullptr)
		{
			delete[]this->tehnologieInvatata;
		}
	}

	PomicultorAvansat& operator=(const PomicultorAvansat& pa)
	{
		if (this != &pa)
		{
			Pomicultor::operator=(pa);
			this->aniStudiu = pa.aniStudiu;
			if (this->tehnologieInvatata != nullptr)
			{
				delete[]this->tehnologieInvatata;
	...
	friend ostream& operator << (ostream& out, const PomicultorAvansat & pa)
	{
		out << Pomicultor(pa) << endl;
		out << "Pomicultorul are " << pa.aniStudiu << " ani de studiu in domeniu." << endl;
		out << "Tehnologia aprofundata este: " << pa.tehnologieInvatata << endl;
		return out;
	}

	int getAniStudiu() const
	{
		return aniStudiu;
	}

	char* getTehnologieInvatata() const
	{
		return tehnologieInvatata;
	}

	void setAniStudiu(int aniStudiu)
	{
		if (this->aniStudiu > 0)
		{
			this->aniStudiu = aniStudiu;
...
int main()
{
	...
	LivadaEco le1;
	LivadaEco le2("Gradinarit organic", 7.6, 4321, 3, new float[3] {4, 4, 4});
	LivadaEco le3 = le1;
	cout << le1 << endl << le2 << endl << le3 << endl;
	le3 = le2;
	cout << le3 << endl;
	le2.setCalitateSol(9.2);
	cout << "Getteri livada eco : " << endl << le2.getCalitateSol() << endl << le2.getEcologieFolosita() << endl;

	PomicultorAvansat pa1;
	PomicultorAvansat pa2(5, "Pomicultura si conservare", "Daniel", 39, true);
	PomicultorAvansat pa3 = pa1;
	cout << pa1 << endl << pa2 << endl << pa3 << endl;
	pa3 = pa2;
	pa2.setAniStudiu(3);
	cout << pa3 << endl << "Getteri pomicultor avansat: "<<pa2.getAniStudiu() << endl << pa2.getTehnologieInvatata() << endl;
};